...
Heat_map <- function(SM_DF,Accession = "B73"){
  #3.1.Merge Assay----
  
  pos_mat <- GetAssayData(SM_DF, assay = "Metabolism_pos", slot = "data")
  neg_mat <- GetAssayData(SM_DF, assay = "Metabolism_neg", slot = "data")
  ...
  new_assay <- CreateAssayObject(counts = merged_mat)
  SM_DF[["Metabolism_all"]] <- new_assay
  
  #3.2.Extract Metabolism Cell----
  
  # 获取 Metabolism_neg 中有数据的细胞
  valid_cells <- colnames(SM_DF[["Metabolism_all"]])
  
  # 在 Seurat 对象中保留这些细胞
  SM_DF_sub <- subset(SM_DF, cells = valid_cells)
  
  # 传入features参数
  features_use <- features_all[features_all %in% rownames(SM_DF_sub[["Metabolism_all"]])]
  ...
  SM_DF_sub@meta.data$Clusters <- SM_DF_sub@meta.data$newclusters
  ...
  SM_DF_sub@meta.data$Clusters <- factor(SM_DF_sub@meta.data$Clusters, levels = desired_order)
...
B73_Heatmap <- Heat_map(SM_DF = B73_se
                        ,Accession = "B73")
...
Heat_map <- function(SM_DF,Accession = "Teo"){
  #3.1.Merge Assay----
...
Teo_Heatmap <- Heat_map(SM_DF = Teo_se
                        ,Accession = "Teo")
